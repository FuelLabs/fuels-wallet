...
describe('TxIcon', () => {
  it('a11y', async () => {
    await testA11y(<TxIcon transaction={MOCK_TRANSACTION_SCRIPT} />);
  });

  it('a11y Loader', async () => {
    await testA11y(<TxIcon.Loader />);
  });

  it('should show transaction Script', async () => {
    render(<TxIcon transaction={MOCK_TRANSACTION_SCRIPT} />);

    expect(screen.getByText(/Script/i)).toBeInTheDocument();
  });

  it('should show transaction Create', async () => {
    render(<TxIcon transaction={MOCK_TRANSACTION_CREATE} />);
