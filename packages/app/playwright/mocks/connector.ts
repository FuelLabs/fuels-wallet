...
export const mockConnectorVersion = {
  app: '0.0.1',
  network: '>=0.12.4',
};
...
export class MockConnector extends FuelConnector {
  ...
  constructor(options: MockConnectorOptions = {}) {
    ...
    if (options.wallets) {
      ...
      this._accounts =
        options.accounts ??
        new Array(2).fill(0).map(() => Address.fromRandom().toString());
    }
  ...
  async ping() {
    await new Promise((resolve) => setTimeout(resolve, this._pingDelay));
    return true;
  }

  async version() {
    return mockConnectorVersion;
  }
