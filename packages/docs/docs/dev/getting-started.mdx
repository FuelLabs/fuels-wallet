...
Alternatively, you can directly use the SDK by creating a new instance of `Fuel` to interact with the wallet.
...
It's possible that your application loads before `window.fuel` is injected. To detect when `fuel` is loaded and ready to use, you can listen to the event `FuelLoaded` on the document.
