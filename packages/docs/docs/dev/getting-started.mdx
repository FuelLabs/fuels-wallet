...
To install a pre-release of `@fuel-wallet/sdk` with a specific commit, use the command:

```bash
npm install fuels @fuel-wallet/sdk@<version>-<branch>-<commit-hash>
```

Substitute `<version>`, `<branch>`, and `<commit-hash>` accordingly. Check the [npm version history](https://www.npmjs.com/package/@fuel-wallet/sdk?activeTab=versions) for the exact identifiers.
