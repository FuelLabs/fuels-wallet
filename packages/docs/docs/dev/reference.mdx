...
For checking if a connector is on the connectors list use `hasConnector(connectorName: string): boolean` method.
...
This method will check if the desired connector is install if not detect in 1 second it throws.
...
This method not triggers erros if the target Connector is not install.
...
For removing a Wallet Connector use the `removeConnector(connectorName: string): boolean` method.
