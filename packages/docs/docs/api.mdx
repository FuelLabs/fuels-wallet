...
We can listen for a new connection to a node. The callback is passed the connection object.
...
We can listen for when the client connects his accounts. The callback is passed the account object.

```js
fuel.on(fuel.events.ACCOUNTS, async (account) => {});
```

### Current Account

We can listen for when the client is authorized to use an account. If there are multiple accounts, the current account on the wallet is returned. The callback is passed the account object.
...
We can listen for when the client connects to a network. The callback is passed the network object.
