...
Before using our SDK, you need to install the Fuel Wallet browser extension.
Since we're only in an alpha version it isn't possible to download directly from the Chrome Extensions Store.
You need to download the [FuelWallet zip file](https://wallet.fuel.network/app/fuel-wallet.zip), and then import it directly into your extensions management page on Chrome or Brave.
